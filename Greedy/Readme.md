...
OrderedDict 활용해서 index가 앞에 있을수록 안 쓴지 오래된것, 뒤에 있을수록 쓴지 별로 안된 것이다. 사용할때마다 사용한 키를 OrderDict에 있는 move_to_end로 뒤에 보내주면 된다.
참고 [OrderDict](https://docs.python.org/3/library/collections.html#collections.OrderedDict)

### 마감기한이 있는 최적 스케줄러 짜기

이득순으로 정렬 후, unionfind를 응용해서 마감기한에 따른 최적 스케줄러를 짠다. 이렇게 할 경우 시간복잡도는 nlogn으로 풀 수 있다.
보통 n^2소요
...
### reconstructQueue(키에 따른 대기열 재구성) - LeetCode_406

여기서 중요한 포인트는 자신의 키 이상이기 때문에 키가 큰 사람부터 고려하면 된다.
(자신보다 키가 작은 사람은 앞에 몇명이 있든 상관 없으므로)
h = 그 사람의 키, k는 사람들 중 자신의 키 이상인 사람들의 수라고 한다.
[h,k]로 이루어져 있는 [[7,0],[4,4],[7,1],[5,0],[6,1],[5,2]]에서
키 큰 사람들 부터 k 번째에 insert해주면 된다.
그 이유는 자신의 차례때, 이전까지 들어온 사람들은
자신보다 전부 키가 크기 때문에 k가 자신의 위치(index)가 되어야한다.
키가 7일때
[7,0], [7,1] 이렇게 대기열을 구성하고
키가 6일때 [6,1]은
지금까지의 대기열은 자신보다 전부 키가 크기 때문에 index 1에 들어가야한다.
[7,0], [6,1], [7,1]
...

### PAI_79 - leastInterval(태스크 스케줄러)

매 타임마다 쌓여있는 블록을 큰 순서대로 정렬해서
큰 순서대로 태스크 간격만큼 하나씩만 제거해주면 된다.
제거해줄때마다 간격을 세주고 또한 남아있는것도 간격으로 카운트해준다.
