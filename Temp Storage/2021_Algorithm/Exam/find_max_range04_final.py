def find_max_range(nums):
    ...
    left = mid_to_left_max = mid_to_left_min = 0
    for l in range(mid - 1, -1, -1):
        left += nums[l]
        mid_to_left_max = max(left, mid_to_left_max)
        mid_to_left_min = min(left, mid_to_left_min)

    right = mid_to_right_max = mid_to_right_min = 0
    for r in range(mid + 1, len(nums)):
        right += nums[r]
        mid_to_right_max = max(right, mid_to_right_max)
        mid_to_right_min = min(right, mid_to_right_min)
    ...
    mid_sum = 0
    for num in (mid_to_left_min, nums[mid], mid_to_right_min):
        mid_sum += num
        mid_sum_max = max(abs(mid_sum), mid_sum_max)
