...
def dfs(is_a_turn, aloc, bloc, board, move_cnt):
    ...
    for dy, dx in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
        ...
        if 0 <= my < len(board) and 0 <= mx < len(board[0]) and board[my][mx]:
            ...
            # 자기가 이기는 경우만 담는다.
            if is_a_turn:
                r = dfs(0, [my, mx], bloc, board, move_cnt + 1)
                if r[0] == 1:
                    ret.append(r)
            else:
                r = dfs(1, aloc, [my, mx], board, move_cnt + 1)
                if r[0] == -1:
                    ret.append(r)
    ...
    for r, move in ret:
        check_sum += r
        if r == 1:
            a_max_move_cnt = max(a_max_move_cnt, move)
        elif r == -1:
            b_max_move_cnt = max(b_max_move_cnt, move)

    # 자신이 이기는 경우 없다면
    if not ret:
        return (0, 0)
    # 모두 a가 이긴 경우가 1개라도 있으면?
    if check_sum > 0:
        return (1, a_max_move_cnt)
    elif check_sum < 0:  # 모두 b가 이긴 경우
        return (-1, b_max_move_cnt)
    return (0, 0)  # a가 이길수도, b가 이길수도 있던 상황
