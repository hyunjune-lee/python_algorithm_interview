...
class Solution:
	def findMinHeightTrees(self, n: int, edges: List[List[int]]) -> List[int]:
		...
		for a,b in edges:
			dic[a].append(b)
			dic[b].append(a)
		# 많은 것부터 해서 최대 높이가 달라지면 최적화..일거 같으나 모두 체크하자
		min_depth_list = []
		dic_by_len = sorted(dic.items(), key =  lambda x: len(x[1]), reverse=True)
		min_depth = n + 1
		for i, _ in dic_by_len:
			Q = deque([i])
			depth = 0
			# 중복체크
			path = []
			...
			if min_depth == depth:
				min_depth_list.append(i)

		return min_depth_list
